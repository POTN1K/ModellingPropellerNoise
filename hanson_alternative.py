...
class Propeller:
    ...
    def angle_of_attack(self, z):  # different from that group but not that different
        return (self.twist(z) - (np.arctan(self.flow.M_x / (self.M_r(z))) * 180 / np.pi)) * np.pi / 180
    ...
    def psi_D_derivative(self, x, args):
        ...
        return Drag_sec * np.exp(1j * self.k_x_static * x)
    ...
    def psi_L_derivative(self, x, args):
        ...
        return Lift_sec * np.exp(1j * self.k_x_static * x)

    def psi_V_derivative(self, x, args):
        # z = args[0]
        # m = args[1]
        return (self.thicknessDistNormalized(x + 0.5)[0] - self.thicknessDistNormalized(x + 0.5)[1]) * np.exp(
            1j * self.k_x_static * x)
    ...
    def p_Vm_derivative(self, z, m):
        self.k_x_static = self.k_x(z, m)
        self.aoa = self.angle_of_attack(z)
        P_Vm_1 = (self.M_r(z)) ** 2 * math2.besselsFunc(m * self.bladeNumber,
                                                        m * self.bladeNumber * z * self.M_t * np.sin(self.theta) / (
                                                            self.dopplerFactor))
        P_Vm_2 = self.k_x_static ** 2 * (self.thicknesstoCord) * self.psi_V(z, m)
        # P_Vm_2 = self.k_x(z, m) ** 2 * (self.thicknessDistfull(z)) * self.psi_V(z, m)
    ...
    def p_Dm_derivative(self, z, m):  # 🐙🐙🐙🐙🐙🐙🐙
        ...
        # print(type(self.areaDrag(z)))
        # print(type(self.dragArea))
        self.k_x_static = self.k_x(z, m)

        # C_D = self.Drag(z)
        C_D = self.dragArea
        # if ((self.dragArea != self.areaDrag(z))):print(str(self.dragArea) + " " + str(self.areaDrag(z)) + " 1")
        P_Dm_1 = (self.M_r(z)) ** 2 * math2.besselsFunc(m * self.bladeNumber,
                                                        m * self.bladeNumber * z * self.M_t * np.sin(self.theta) /
                                                        (self.dopplerFactor))
        P_Dm_2 = 1j * self.k_x_static * (C_D / 2) * self.psi_D(z, m)
        # P_Dm_2_alt = 1j * self.k_x_static * (C_D / 2) * self.psi_D2(z, m)
    ...
    def p_Lm_derivative(self, z, m):
        ...
        P_Lm_1 = (self.M_r(z)) ** 2 * math2.besselsFunc(m * self.bladeNumber,
                                                        m * self.bladeNumber * z * self.M_t * np.sin(self.theta) /
                                                        (self.dopplerFactor))
        P_Lm_2 = 1j * k_y * (C_L / 2) * self.psi_L(z, m)
        # P_Lm_2_alt = 1j * k_y * C_L / 2 * self.psi_L2(z, m)
    ...
    def Cp_u(self, x, z):
        ...
        return float(self.Cpu(x, z))

    def Cp_l(self, x, z):  # 😎😎😎😎
        # to be determined
        return float(self.Cpl(x, z))
    ...
    def A_chord(self, z, x):
        ...
        dA = self.Cp_u(x + 0.5, z) * self.thicknessDistDerivative(x + 0.5)[0] - self.Cp_l(x + 0.5, z) * \
             self.thicknessDistDerivative(x + 0.5)[1]
        # print(self.thicknessDistDerivative(x+0.5)[0])
        return dA
    ...
    def liftDist(self, x, z):
        dN = self.N_chord(z, x)
        dA = self.A_chord(z, x)
        dL = dN * np.cos(self.aoa) - dA * np.sin(self.aoa)
    ...
    def areaLift(self, z):
        area = math2.integration(self.liftDist, -0.49999, 0.5, 400, z, 'Simpsons')
        # print('AOA'+ str(self.angle_of_attack(z)))
        # print('Area' + str(area))
        return area
    ...
    def dragDist(self, x, z):
        dN = self.N_chord(z, x)
        dA = self.A_chord(z, x)
        dD = dN * np.sin(self.aoa) + dA * np.cos(self.aoa)
        # print(dD)
...
# middleMic = Propeller(flow, 8000, 2, 0.3, 0, np.pi/2, 1.2)
# print("middle Mic harmonic 1: " + str(middleMic.noise(1)))
